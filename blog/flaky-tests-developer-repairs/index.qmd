...
and fail at random, disrupting your continuous integration process and wasting
valuable development time. While automated tools promise to help detect these
problematic tests, how well do they actually work for finding the flaky tests
...
by automated rerunning tools, or they present results without any baseline at
all. But here's the issue: these approaches don't tell us whether the detected
flaky tests are actually ones that developers would spend time fixing in
practice.

Our study addresses this gap by using a baseline derived from actual developer
behavior — 75 commits where human software developers repaired flaky tests in 31
real-world Python projects. This developer-based methodology provides a more
realistic assessment of detection technique usefulness.
...
- **Noise Improves Detection**: Rerunning with environmental noise (randomizing
thread priorities, test execution order, network speeds, and Python versions)
performed significantly better than plain rerunning, improving recall from 21%
to 40%. This supports the importance of testing under diverse conditions.

- **Common Causes and Repairs**: Through manual analysis, we found that
randomness was the most frequent cause of flaky tests (appearing in 23% of
commits), followed by asynchronous waiting issues and timing problems. The most
common developer repair was widening assertion ranges to accept broader value
ranges (25% of commits).
