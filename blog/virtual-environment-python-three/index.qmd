...
another. <span class="gist">Since virtual environments ensure that all Python
  projects are no longer forced to depend on a system-wide library, they
  support program isolation and thus enable software engineers to better manage
  the library dependencies.</span>
...
<span class="gist">What is this new approach? It is called `venv`! While there
  is some great documentation on
  [venv](https://docs.python.org/3/library/venv.html), I decided to write this
  post to share some of the additional details that I learned about how to use
  it on a recent version of the Ubuntu operating system.</span> I hope that
...
is `python3 -m venv pylightningy`. This command will create a `venv`-based
...
command prompt that the `pylightning` project is now active.
...
you have returned to the prior command-line environment.
...
using [jdxcode/rtx](https://github.com/jdxcode/rtx) to install programming
languages like Python. Try it out!
