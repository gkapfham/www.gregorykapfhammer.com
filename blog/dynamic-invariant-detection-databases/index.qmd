...
introduce a novel approach that does just that! The presented method
...
- **Technique**: A new technique that extends the notion of dynamic invariant
detection to SQL-based relational databases and the applications that employ
them.

- **Tool**: An implementation that analyzes existing databases and the Java
applications that utilize these databases to automatically produce files for
input to Daikon.
...
database-specific application for schema constraint modification.

## Results

The results of the empirical studies demonstrate the feasibility of collecting
meaningful invariants for four fixed data sets and three subject programs.
Furthermore, the research shows that dynamic invariants for relational databases
can be effectively used to identify areas where the database schema constraints
could be tightened to protect data integrity.
